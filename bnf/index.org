...
<literal>        ::= '"' <text> '"' | "'" <text> "'"
  ...
  that never appears on LFS are /terminals/, e.g., =<rule-name>= and
  =<text>=.  Terminal, as its name suggests, is the elementary symbols
  of the languages being represented.  It may not be changed by the
  derivation rules, since it never appears on the LHS.
