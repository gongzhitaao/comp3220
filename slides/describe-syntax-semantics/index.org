...
*** Attribute Grammar
    ...
    Attribute Grammar consists two part
...
*** Example Attribute Grammer

    Given the attribute grammar

    #+BEGIN_SRC bnf
<stmt> ::= <var> "=" <expr> (stmt.val = val.val = expr.val)
<expr> ::= <var>            (expr.val = var.val | var.val + var.val)
         | <var> "+" <var>
<var>  ::= <bin>            (var.val = bin.val)
<var>  ::= <bin1> <bin2>   (var.val = bin1.val * 2 + bin2.val)
<bin>  ::= 0 | 1            (bin.val = 0 | 1)
    #+END_SRC
